...
def choose_video(videos):
    # Adaptive quality: pick first under size limit
    for url in videos:
...
def upload_stream(video_url):
    # stream download into memory and upload to YouTube
    ...
    body = {"snippet": {"title": f"Trending: {TREND_KEYWORD}", "description": video_url},
...
def main():
    ...
    for term in trends:
        ...
        all_videos.append(pick)
    # Save log
    with open("output.txt", "w", encoding="utf-8") as f:
        json.dump(all_videos, f, ensure_ascii=False, indent=2)
    print("Videos selected:", all_videos)
    # Upload each
